      ...
      fetch("/api/transfer-requests",{credentials:"same-origin"})
        ...
        .then(function(d){
          ...
          tl.innerHTML=reqs.map(function(r){
            return '<div class="contact-card"><strong>'+esc(r.cat_name)+'</strong><p class="muted" style="font-size:.85rem">Request from: '+esc(r.requester_email)+'</p>'+(r.message?'<p style="font-size:.85rem">'+esc(r.message)+'</p>':'')+'<div style="display:flex;gap:.5rem;margin-top:.5rem"><button class="btn-primary accept-btn" data-id="'+esc(r.id)+'">Accept</button><button class="btn-warn decline-btn" data-id="'+esc(r.id)+'">Decline</button></div><p class="muted status-msg" style="font-size:.85rem;margin:.5rem 0 0"></p></div>'
          }).join("");
