...
<body>
  ...
  <script>
    (function(){
      ...
      vaccineForm.addEventListener("submit",function(e){e.preventDefault();var f=e.target;var nextDate=f.next_vaccine_date&&f.next_vaccine_date.value||null;postJson("/api/cats/${safeId}/vaccines",{vaccine_name:f.vaccine_name.value,date_given:f.date_given.value}).then(function(r){if(r.ok){if(nextDate)postJson("/api/cats/${safeId}/update",{next_vaccine_date:nextDate}).catch(function(){});location.reload();}else r.text().then(alert);});});
...
function renderForms(publicId: string, lang: LanguageCode): string {
  return `<h2>${t(lang, "addVaccine")}</h2><form id="vaccine-form"><label>${t(lang, "vaccineName")}<input name="vaccine_name" required maxlength="100" /></label><label>${t(lang, "dateGiven")}<input name="date_given" type="date" /></label><label>${t(lang, "nextVaccineDateInForm")}<input name="next_vaccine_date" type="date" /></label><button class="btn" type="submit">${t(lang, "save")}</button></form>
